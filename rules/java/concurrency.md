...
> 基于《阿里巴巴 Java 开发手册》p3c 第五章"并发编程"。语言无关原则见 [common/concurrency.md](../common/concurrency.md)。
...
- **Virtual Thread 下禁止 `synchronized`** — `synchronized` 会导致 Virtual Thread pinning 到平台线程，改用 `ReentrantLock`（详见 [coding-style.md](./coding-style.md) Virtual Threads 段）
