...
4. **循环验证** — 每个阶段定义可验证成功标准，不达标不回；覆盖率≥60%（默认）、CRITICAL=0 是底线
...
1. `Agent({ subagent_type: "tdd-guide" })`，prompt 必须包含：
   ...
   - **要求产出**：测试文件路径 + 覆盖率报告
2. 走 RED → GREEN → IMPROVE，**默认覆盖率 ≥ 60%**
3. Agent 调用失败 → **展示错误，暂停等用户决定**（两种模式都停）。
4. 展示覆盖率报告，**告知用户当前覆盖率，暂停询问**：「当前覆盖率 XX%。是否继续提高到 80%？」如用户确认，继续优化至 ≥80%。用户说"够了"则继续下一阶段。
   - **自动模式**：60% 达标后不暂停，直接继续。覆盖率未达 60% 则提示并暂停。
