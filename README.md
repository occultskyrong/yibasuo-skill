...
> **基于 v2.17 实测估算**（CodeGraph + 渐进式规范路由后的数据）。额外成本换取：架构评审 + 强制 TDD（覆盖率≥80%）+ 代码审查 + 安全审查 + 基础设施审查 + 规范对照检查。如需进一步降本，启用 CodeGraph 可减少 ~62% 工具调用和 ~25% 总 token。
